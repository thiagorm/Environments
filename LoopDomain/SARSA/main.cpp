...
void main()
{
	...
	while(count != convergence_time)
	{
		...
		if(actual_state == 0 && action == 0)
		{
			...
			nextQValue = states[next_state].actions[nextAction];
			states[actual_state].actions[action] = SARSACalculate(states[actual_state].actions[action], nextQValue, reward, alfa, gamma);
			total_reward += reward;
		...
		else if(actual_state == 1 || actual_state == 2 || actual_state == 3 || actual_state == 4)
		{
			if(actual_state == 4)
			{
				...
				nextAction = getAction(states[next_state], eGreedy);
				nextQValue = states[next_state].actions[nextAction];
				states[actual_state].actions[0] = SARSACalculate(states[actual_state].actions[0], nextQValue, min_reward, alfa, gamma);
				states[actual_state].actions[1] = SARSACalculate(states[actual_state].actions[1], nextQValue, min_reward, alfa, gamma);
			...
			else
			{
				...
				nextAction = getAction(states[next_state], eGreedy);
				nextQValue = states[next_state].actions[nextAction];
				states[actual_state].actions[0] = SARSACalculate(states[actual_state].actions[0], nextQValue, reward, alfa, gamma);
				states[actual_state].actions[1] = SARSACalculate(states[actual_state].actions[1], nextQValue, reward, alfa, gamma);
				total_reward += reward;
				AcaoA += states[actual_state].actions[0];
				AcaoB += states[actual_state].actions[1];
			}
		...
		else if(actual_state == 5 || actual_state == 6 || actual_state == 7 || actual_state == 8)
		{
			...
			if(actual_state == 8)
			{
				...
				nextAction = getAction(states[next_state], eGreedy);
				nextQValue = states[next_state].actions[nextAction];
				states[actual_state].actions[0] = SARSACalculate(states[actual_state].actions[0], nextQValue, max_reward, alfa, gamma);
				states[actual_state].actions[1] = SARSACalculate(states[actual_state].actions[1], nextQValue, max_reward, alfa, gamma);
