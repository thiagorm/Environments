...
void elegibilityTraces(float QValue, float reward, float gamma, struct State z[], int actual_state, int action, struct State grid[], float alfa, float lambda, int nextAction, int AStar,
	int next_state)
{
	float delta = 0;
	int m = 0;
	...
	delta = reward + ((gamma * grid[next_state].actions[AStar]) - QValue);
...
void main()
{
	...
	int actual_state = 0, next_state = 0, Astar = 0;
	...
	while(count != convergence_time)
	{
		...
		if(actual_state == 0 && action == 0)
		{
			...
			Astar = verifyMaiorAction(states[next_state]);
			elegibilityTraces(states[actual_state].actions[action],reward,gamma,z,actual_state,action,states,alfa,lambda,nextAction,Astar,next_state);
		...
		else if(actual_state == 1 || actual_state == 2 || actual_state == 3 || actual_state == 4)
		{
			if(actual_state == 4)
			{
				...
				nextAction = getAction(states[next_state],eGreedy);
				Astar = verifyMaiorAction(states[next_state]);
				elegibilityTraces(states[actual_state].actions[0],min_reward,gamma,z,actual_state,0,states,alfa,lambda,nextAction,Astar,next_state);
				elegibilityTraces(states[actual_state].actions[1],min_reward,gamma,z,actual_state,1,states,alfa,lambda,nextAction,Astar,next_state);
			...
			else
			{
				...
				nextAction = getAction(states[next_state],eGreedy);
				Astar = verifyMaiorAction(states[next_state]);
				elegibilityTraces(states[actual_state].actions[0],reward,gamma,z,actual_state,0,states,alfa,lambda,nextAction,Astar,next_state);
				elegibilityTraces(states[actual_state].actions[1],reward,gamma,z,actual_state,1,states,alfa,lambda,nextAction,Astar,next_state);
				total_reward += reward; //* pow(gamma, count);
			}
		...
		else if(actual_state == 5 || actual_state == 6 || actual_state == 7 || actual_state == 8)
		{
			...
			if(actual_state == 8)
			{
				...
				nextAction = getAction(states[next_state],eGreedy);
				Astar = verifyMaiorAction(states[next_state]);
				elegibilityTraces(states[actual_state].actions[0],max_reward,gamma,z,actual_state,0,states,alfa,lambda,nextAction,Astar,next_state);
				elegibilityTraces(states[actual_state].actions[1],max_reward,gamma,z,actual_state,1,states,alfa,lambda,nextAction,Astar,next_state);
