	...
	int nexti, int nextj)
{
	...
	//max_a = maxQValue;
	delta = reward + ((gamma * grid[nexti][nextj].actions[nextAction]) - value_actual_state);
	...
	for(i = 0; i < number_lines; i++)
	{
		for(j = 0; j < number_col; j++)
		{
			if(j == (number_col-1))
			{
				...
				gotoxy(x,y);
				//getch();
				printf("%.25f", grid[i][j].actions[0]);
				y += 1;
				gotoxy(x,y);
				printf("%.25f", grid[i][j].actions[1]);
				y += 1;
				gotoxy(x,y);
				printf("%.25f", grid[i][j].actions[2]);
				y += 1;
				gotoxy(x,y);
				printf("%.25f", grid[i][j].actions[3]);
				printf("  ");
				//getch();
				x = flag_x;
				y = flag_y;
