...
const float alfa = 0.1, gamma = 0.95, eGreedy = 0.1;
...
void main()
{
	...
	int count = 0, action = NULL, i = 0, j = 0, k = 0, l = 0, actions = 0, next_action = 0;
	...
	//action = getActionSoftMax(grid[i][j]);
	for(l = 0; l < 30; l++)
	{
	strcpy(name, l+"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.txt");
	while(count != convergence_time)
	{
		...
		//{
				...
				case 0:
					if(i - 1 >= 0)
					{
						if(grid[i-1][j].actions[action] != -999)
						{
							...
							next_action = getAction(grid[i-1][j], eGreedy);
							//printf("next:%d\n", next_action);
							//printf("action:%d\n", action);
							if(i-1 == 0 && j == 8)
							{
								grid[i][j].actions[action] = qLearningCalculate(grid[i][j].actions[action],grid[i-1][j].actions[next_action],reward_goal,alfa,gamma);
								total_reward+=reward_goal * gamma;
							}
							else
							{
								grid[i][j].actions[action] = qLearningCalculate(grid[i][j].actions[action],grid[i-1][j].actions[next_action],reward,alfa,gamma);
						...
						{
							next_action = getAction(grid[i][j], eGreedy);
						}
					}
					else
					{
				...
				case 2:
					if( j - 1 >= 0)
					{
						if(grid[i][j-1].actions[action] != -999)
						{
							...
							next_action = getAction(grid[i][j-1], eGreedy);
							//printf("next:%d\n", next_action);
							//printf("action:%d\n", action);
							if(i == 0 && j-1 == 8)
							{
								grid[i][j].actions[action] = qLearningCalculate(grid[i][j].actions[action],grid[i][j-1].actions[next_action],reward_goal,alfa,gamma);
								total_reward+=reward_goal * gamma;
							}
							else
							{
								grid[i][j].actions[action] = qLearningCalculate(grid[i][j].actions[action],grid[i][j-1].actions[next_action],reward,alfa,gamma);
								total_reward+=reward * gamma;
							}
							j--;
						}
			...
			if(i == 0 && j == 8)
			{
				...
				actions = 0;
				next_action = getActionSoftMax(grid[i][j]);
				//total_reward = 0;
			}
		action = next_action;
