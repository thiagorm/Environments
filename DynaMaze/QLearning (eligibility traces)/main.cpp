...
const float alfa = 0.1, gamma = 0.95, eGreedy = 0.9, lambda = 0.01;
...
void elegibilityTraces(float value_actual_state, float maxQValue, float reward, float gamma, struct State z[][number_col], 
	int i, int j, int action, struct State grid[][number_col], float alfa, float lambda, int nextAction, int AStar, int nexti, int nextj)
{
	float max_a = 0;
	float delta = 0;
	int m = 0;
				...
				case 0:
					if(i - 1 >= 0)
					{
						if(grid[i-1][j].actions[action] != -999)
						{
							...
							nextStatei = i-1;
							nextStatej = j;
							//printf("Value Action: %.2f\n", grid[i][j].actions[action]);
							if(i-1 == 0 && j == 8)
							...
							nextAction = getAction(grid[i][j],eGreedy);
					}
					else 
							nextAction = getAction(grid[i][j],eGreedy);
					actions++;
