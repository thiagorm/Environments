...
void gravarRecompensas(FILE *link, int r, char name[])
{
...
void elegibilityTraces(float QValue, float maxQValue, float reward, float gamma, struct State z[][number_col], int i, int j, int action, 
			...
			case 0:
				...
				if(i - 1 >= 0)
				{
					if(states[i-1][j].actions[action] != wall)
					{
						...
						{
							elegibilityTraces(states[i][j].actions[action],maxQValue,number_collected_flags,gamma,z,i,j,action,states,alfa,lambda, nextAction, Astar, next_state_i, next_state_j);
							total_reward += number_collected_flags;
						}
						else
						{
							elegibilityTraces(states[i][j].actions[action],maxQValue,ordinary_reward,gamma,z,i,j,action,states,alfa,lambda, nextAction, Astar, next_state_i, next_state_j);
