...
const float alfa = 0.8, gamma = 0.8, eGreedy = 0.9, n = 50;
			...
			case 0:
				...
				if(i - 1 >= 0)
				{
					if(states[i-1][j].actions[action] != wall)
					{
						...
						{
							states[i][j].actions[action] = dynaQLearning(states[i][j].actions[action],maxQValue,number_collected_flags,alfa,gamma);
							total_reward += number_collected_flags;
						}
						else
						{
							states[i][j].actions[action] = dynaQLearning(states[i][j].actions[action],maxQValue,ordinary_reward,alfa,gamma);
						...
						i = next_state_i;
						for(pn = 0; pn < n; pn++)
								QPlanning(gridModel,states,actions_taken,visited_location,alfa, gamma);
					}
				}
	...
	for(i = 0; i < number_lines; i++)
	{
		for(j = 0; j < number_col; j++)
		{
			if(j == (number_col-1))
			{
				...
				gotoxy(x,y);
				//getch();
				printf("%.4f", states[i][j].actions[0]);
				y += 1;
				gotoxy(x,y);
				printf("%.4f", states[i][j].actions[1]);
				y += 1;
				gotoxy(x,y);
				printf("%.4f", states[i][j].actions[2]);
				y += 1;
				gotoxy(x,y);
				printf("%.4f", states[i][j].actions[3]);
				printf("  ");
				//getch();
				x = flag_x;
				y = flag_y;
