...
void elegibilityTraces(float QValue, float reward, float gamma, struct State z[], int actual_state, int action, struct State grid[], 
	float alfa, float lambda, int nextAction, int AStar, int next_state)
{
	float delta = 0;
	int m = 0;
	int n = 0;
...
void main()
{
	...
	int l = 0, teste = 0,  acaoA = 0, acaoB = 0, maiorAcao = 0, tr;
		...
		switch(action)
		{
			...
			case 0:
				if(actual_state == 4)
				{
					...
					elegibilityTraces(states[actual_state].actions[action],max_reward_a,gamma,z,actual_state,action,states,alfa,lambda,nextAction,Astar,next_state);
				...
				else
				{
					...
					nextAction = getAction(states[next_state],eGreedy);
					Astar = verifyMaiorAction(states[next_state]);
					elegibilityTraces(states[actual_state].actions[action],min_reward_a,gamma,z,actual_state,action,states,alfa,lambda,nextAction,Astar,next_state);
			...
			case 1:
				...
				nextAction = getAction(states[next_state],eGreedy);
				Astar = verifyMaiorAction(states[next_state]);
				elegibilityTraces(states[actual_state].actions[action], reward_b,gamma,z,actual_state,action,states,alfa,lambda,nextAction,Astar,next_state);
